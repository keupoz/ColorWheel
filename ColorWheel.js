...
function ColorWheel (callback, size = 256) {
 ...
 center, radius,
 spectreThickness, spectreSection,
 triangleRadius, triangleBorder, triangleSide, triangleHeight,
 saturationGradient, brightnessGradient,
 
 hueRad = 0,
 toneX, toneY, toneRadius,
 ...
 function updateToneCursor () {
  if (CurrentSaturation) {
   let
   saturationRad = CurrentSaturation * RAD_60,
   radius        = CurrentBrightness * triangleHeight / Math.cos(saturationRad - RAD_30),
   angle         = RAD_90 - saturationRad;
   
   toneX =  radius * Math.cos(angle) + vertices[2].x;
   toneY = -radius * Math.sin(angle) - vertices[2].y;
  } else {
   toneX = vertices[1].x;
 ...
 function makePointRelative (point) {
  point[0] =  point[0] - output.canvas.offsetLeft - center;
  point[1] = -point[1] + output.canvas.offsetTop  + center;
 ...
 function moveTone (point, start) {
  ...
  if (relativeX < 0) {
   ...
   let
   saturationRad = Math.acos(saturationCos),
   brightnessRad = Math.acos(brightnessCos);
   
   if (brightnessRad > RAD_60 && brightnessRad % RAD_30) CurrentSaturation = brightnessHypot * Math.cos(brightnessRad - RAD_60) / triangleSide;
   else CurrentSaturation = saturationRad / RAD_60;
   if (saturationRad > RAD_60 && saturationRad % RAD_30) CurrentBrightness = saturationHypot * Math.cos(saturationRad - RAD_60) / triangleSide;
   else CurrentBrightness = saturationHypot * Math.cos(saturationRad - RAD_30) / triangleHeight;
 ...
 function getMoveHandler (point) {
  let cursorDistance = Math.hypot(point[0], point[1]);
  // Spectre wheel
 ...
 function registerEvent (start, move, end, cancel, getPoint) {
  output.canvas.addEventListener(start, function (e) {
   let
   point   = makePointRelative(getPoint(e)),
 ...
 this.setSize = function (value) {
  background.canvas.width = background.canvas.height = 
  triangle.canvas.width = triangle.canvas.height = 
  output.canvas.width = output.canvas.height = value;
 ...
 registerEvent('touchstart', 'touchmove', 'touchend', 'touchcancel', e => [e.changedTouches[0].pageX, e.changedTouches[0].pageY]);
 registerEvent('mousedown', 'mousemove', 'mouseup', 'mouseleave', e => [e.pageX, e.pageY]);
